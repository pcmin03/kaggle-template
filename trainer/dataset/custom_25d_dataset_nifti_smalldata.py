...
class custom_25d_dataset_nifti_smalldata(torch_data.Dataset):
    def __init__(self,
        ...
        self.spatial = tio.Compose([
                        tio.RandomElasticDeformation(num_control_points=(5,5,5),locked_borders=2,p=0.5),
                        tio.RandomAnisotropy(axes=(0, 1, 2),p=0.5),
                        tio.RandomAffine(0.2,p=0.5),
                        tio.RandomSwap((5,5,5),p=0.5),
                        # tio.CropOrPad(target_shape=(40,100,100)),
    ...
    def load_input(self, index):
        ...
        inputs = []
        # zdim = random.randrange(15,20)
        zdim = random.randrange(10,11)
        for i in self.conf["mri_types"]:
            inputs.append(np.load(sampled_imgs[i])[zdim:zdim+self.conf['N_sample'],:,:])
            # inputs.append(np.load(sampled_imgs[i]))
        
        return np.concatenate(inputs,axis=1)[None]
