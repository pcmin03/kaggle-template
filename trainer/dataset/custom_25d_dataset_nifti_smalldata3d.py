...
class custom_25d_dataset_nifti_smalldata(torch_data.Dataset):
    def __init__(self,
        ...
        self.spatial = tio.Compose([
                        ...
                        tio.CropOrPad(target_shape=(80,192,192)),
    ...
    def load_input(self, index):
        ...
        for i in self.conf["mri_types"]:
            singlinput = np.load(sampled_imgs[i])[zdim:zdim+self.conf['N_sample'],:,:])
        
            if mode == 'train': 
                inputs.append(self.spatial(self.preprocessing(np.array(singlinput)))/255.)
            elif mode == 'valid': 
                inputs.append(self.preprocess(np.array(inputs))/255.)
            else : 
                inputs.append(self.preprocess(np.array(inputs))/255.)
        return inputs
    ...
    def __getitem__(self, index):
        inputs = np.array(self.load_input(index))
        print(inputs.shape)
        if self.mode != "test":
